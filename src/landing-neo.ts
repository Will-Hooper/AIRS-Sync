...
import type { Language, OccupationListPayload, OccupationQueryParams, OccupationRow, SummaryPayload } from "./types/airs.js";
...
interface HomeState extends OccupationQueryParams {
  region: string;
  majorGroup: string;
  label: string;
  q: string;
  ...
  rows: OccupationRow[];
  lang: Language;
  meta: Pick<OccupationListPayload, "dates" | "regions" | "labels" | "groups"> | null;
  ...
  panX: number;
  panY: number;
  hoverQuadrant: string | null;
  hoverPointerX: number;
  hoverPointerY: number;
...
const els = {
  riskTickerTrack: byId<HTMLElement>("riskTickerTrack"),
  dateSelect: byId<HTMLSelectElement>("dateSelect"),
  regionSelect: byId<HTMLSelectElement>("regionSelect"),
  majorGroupSelect: byId<HTMLSelectElement>("majorGroupSelect"),
  labelSelect: byId<HTMLSelectElement>("labelSelect"),
  searchInput: byId<HTMLInputElement>("searchInput"),
  searchSuggestions: byId<HTMLElement>("searchSuggestions"),
  visitorTime: byId<HTMLElement>("visitorTime"),
  visitorPlace: byId<HTMLElement>("visitorPlace"),
  lastUpdatedLabel: byId<HTMLElement>("lastUpdatedLabel"),
  dataModeLabel: byId<HTMLElement>("dataModeLabel"),
  environmentLabel: byId<HTMLElement>("environmentLabel"),
  ...
  avgAirs: byId<HTMLElement>("avgAirs"),
  highRiskCount: byId<HTMLElement>("highRiskCount"),
  occupationCount: byId<HTMLElement>("occupationCount"),
  zoomIndicator: byId<HTMLElement>("zoomIndicator"),
  restructuringRate: byId<HTMLElement>("restructuringRate"),
  augmentingRate: byId<HTMLElement>("augmentingRate"),
  mostChangedOccupation: byId<HTMLElement>("mostChangedOccupation"),
  mostChangedDelta: byId<HTMLElement>("mostChangedDelta"),
  occupationUniverse: byId<HTMLElement>("occupationUniverse"),
  occupationCanvas: byId<HTMLElement>("occupationCanvas"),
  heroFocusTitle: byId<HTMLElement>("heroFocusTitle"),
  heroFocusScore: byId<HTMLElement>("heroFocusScore"),
  heroFocusLabel: byId<HTMLElement>("heroFocusLabel"),
  detailTitle: byId<HTMLElement>("detailTitle"),
  detailScore: byId<HTMLElement>("detailScore"),
  detailLabel: byId<HTMLElement>("detailLabel"),
  detailSummary: byId<HTMLElement>("detailSummary"),
  detailSoc: byId<HTMLElement>("detailSoc"),
  focusGroup: byId<HTMLElement>("focusGroup"),
  ...
  quadrantTooltip: byId<HTMLElement>("quadrantTooltip"),
  quadrantTooltipTag: byId<HTMLElement>("quadrantTooltipTag"),
  quadrantTooltipTitle: byId<HTMLElement>("quadrantTooltipTitle"),
  quadrantTooltipText: byId<HTMLElement>("quadrantTooltipText"),
  ...
  signalSelectionTitle: byId<HTMLElement>("signalSelectionTitle"),
  signalSelectionSummary: byId<HTMLElement>("signalSelectionSummary"),
  detailLink: byId<HTMLAnchorElement>("detailLink"),
  portalButton: byId<HTMLButtonElement>("portalButton"),
  downButton: byId<HTMLButtonElement>("downButton"),
  languageButtons: queryAll<HTMLButtonElement>("[data-lang-option]"),
  viewButtons: queryAll<HTMLButtonElement>("[data-view-mode]"),
  storyCards: queryAll<HTMLElement>("[data-story-step]"),
  storyStageTitle: byId<HTMLElement>("storyStageTitle"),
  storyStageText: byId<HTMLElement>("storyStageText"),
  storyStageIndex: byId<HTMLElement>("storyStageIndex"),
  storyStageTotal: byId<HTMLElement>("storyStageTotal"),
  storyStageView: byId<HTMLElement>("storyStageView"),
  releaseEnvironment: byId<HTMLElement>("releaseEnvironment"),
...
let searchSuggestionHideTimer: number | null = null;
let loadToken = 0;
...
function searchSuggestionSecondary(row: OccupationRow) {
  if (state.lang === "zh") return `${row.socCode} · ${row.title}`;
...
function searchSuggestionRows() {
  if (!state.q || state.loadError) return [];
  return state.rows.slice(0, 8);
}
function searchSuggestionMeta(row: OccupationRow) {
...
function chooseSearchSuggestion(socCode: string) {
...
function renderSearchSuggestions(forceVisible = false) {
  ...
  els.searchSuggestions.querySelectorAll<HTMLButtonElement>(".search-suggestion").forEach((button) => {
...
function currentFocusOffset(zoom: number) {
...
function animateNumber(element: HTMLElement | null, value: number, options: NumberAnimationOptions = {}) {
  if (!element) return;
  const {
    decimals = 0,
    prefix = "",
...
function setOptions(select: HTMLSelectElement, options: string[], label: string, mapper: (value: string) => string = (value) => value) {
...
function computeRiskLayout(rows: OccupationRow[]) {
  const labelRank = { high_risk: 0, restructuring: 1, augmenting: 2, monitor: 3, stable: 4, insulated: 5 };
  const labels = [...new Set(rows.map((row) => row.label))] as string[];
  labels.sort((a, b) => (labelRank[a] ?? 99) - (labelRank[b] ?? 99) || a.localeCompare(b));
  const grouped = new Map<string, OccupationRow[]>(labels.map((label) => [label, []]));
...
function setupLayoutIndices() {
  ...
  nodes.forEach((node, index) => {
    ...
    const existing = node.querySelector<HTMLElement>(".layout-number-badge");
...
function setViewMode(mode: string | undefined, options: ViewModeOptions = {}) {
  if (!mode) return;
  const {
...
function setupReveals() {
  const nodes = document.querySelectorAll<HTMLElement>(".pressure-card, .method-step, .signal-panel, .focus-card, .hero-stat, .story-lead, .story-step-card, .release-card");
  nodes.forEach((node, index) => {
...
function bindActions() {
  els.dateSelect.addEventListener("change", (event) => { state.date = (event.currentTarget as HTMLSelectElement).value; load(); });
  els.regionSelect.addEventListener("change", (event) => { state.region = (event.currentTarget as HTMLSelectElement).value; load(); });
  els.majorGroupSelect.addEventListener("change", (event) => { state.majorGroup = (event.currentTarget as HTMLSelectElement).value; load(); });
  els.labelSelect.addEventListener("change", (event) => { state.label = (event.currentTarget as HTMLSelectElement).value; load(); });
  els.searchInput.addEventListener("input", (event) => {
    state.q = (event.currentTarget as HTMLInputElement).value.trim();
    clearSearchSuggestionHideTimer();
    hideSearchSuggestions();

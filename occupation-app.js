...
function renderTimeline(series) {
  ...
  els.timelineDelta.textContent = `12m ${delta > 0 ? "+" : ""}${delta.toFixed(1)}`;
}
...
async function init() {
  ...
  els.occupationLabel.textContent = labelText(row.label);
  ...
  els.evidenceList.innerHTML = row.evidence.map((item) => `<article class="evidence-item"><h4>Evidence</h4><p>${item}</p></article>`).join("");
  els.taskCloud.innerHTML = row.tasks.map((task) => `<article class="task-chip"><h4>${task.name}</h4><p>Exposure ${task.score.toFixed(2)}</p></article>`).join("");
}

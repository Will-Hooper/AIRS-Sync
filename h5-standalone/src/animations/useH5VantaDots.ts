...
interface VantaDotsOptions {
  ...
  spacingWideViewport?: number;
  spacingNarrowViewport?: number;
  sizeWideViewport?: number;
  sizeNarrowViewport?: number;
  showLines?: boolean;
}
...
export function useH5VantaDots(target: React.RefObject<HTMLElement | null>, options: VantaDotsOptions = {}) {
  useEffect(() => {
    ...
    import("vanta/dist/vanta.dots.min").then((module) => {
      ...
      effect = factory({
        ...
        spacing: window.innerWidth < 768 ? (options.spacingNarrowViewport ?? 34) : (options.spacingWideViewport ?? 24),
        size: window.innerWidth < 768 ? (options.sizeNarrowViewport ?? 2.1) : (options.sizeWideViewport ?? 3),
  ...
  }, [target, options.backgroundColor, options.color, options.color2, options.showLines, options.sizeNarrowViewport, options.sizeWideViewport, options.spacingNarrowViewport, options.spacingWideViewport]);
}

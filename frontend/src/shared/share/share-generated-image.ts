...
export async function shareGeneratedImage(options: {
  ...
  const copied = await copyShareText(options.shareText);
  return {
    status: "fallback",
...
export function getSharePlatformLabel(platform: SharePlatform, language: AppLanguage) {
  if (language === "en") {
...
function getSystemShareMessage(language: AppLanguage, platform: SharePlatform) {
  if (language === "en") {
...
function getCancelMessage(language: AppLanguage) {
  return language === "en" ? "Sharing was cancelled." : "已取消分享。";
}

function getFallbackMessage(language: AppLanguage, platform: SharePlatform, copied: boolean) {
  if (language === "en") {

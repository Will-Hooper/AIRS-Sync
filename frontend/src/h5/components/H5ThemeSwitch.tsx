...
export function H5ThemeSwitch({ language, theme, onChange }: H5ThemeSwitchProps) {
  ...
  return (
    <button
      ...
      title={title}
      className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-3 py-2 transition"
    >
      <span className={`inline-flex items-center justify-center rounded-full px-2 py-1 transition ${theme === "light" ? "bg-gradient-to-r from-[#9ae8d4] to-[#84c6ff]" : "bg-white/5"}`}>
        <SunIcon active={theme === "light"} />
      </span>
      <span className={`inline-flex items-center justify-center rounded-full px-2 py-1 transition ${theme === "dark" ? "bg-gradient-to-r from-[#9ae8d4] to-[#84c6ff]" : "bg-white/5"}`}>

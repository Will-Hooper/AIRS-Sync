...
exports.HUMAN_MOAT_FACTOR_DEFINITIONS = exports.HUMAN_MOAT_QUADRANT_THRESHOLD = exports.AI_REPLACEMENT_PRESSURE_QUADRANT_THRESHOLD = void 0;
exports.normalizeAirsScore = normalizeAirsScore;
exports.calculateHumanMoatScore = calculateHumanMoatScore;
exports.getHumanMoatFactors = getHumanMoatFactors;
exports.getHumanMoatScore = getHumanMoatScore;
exports.getHumanMoatLevel = getHumanMoatLevel;
exports.getHumanMoatLabel = getHumanMoatLabel;
exports.getAiReplacementPressure = getAiReplacementPressure;
exports.getOccupationQuadrant = getOccupationQuadrant;
exports.getQuadrantMetaByKey = getQuadrantMetaByKey;
exports.getOccupationUniverseMetrics = getOccupationUniverseMetrics;
exports.getHumanMoatFactorEntries = getHumanMoatFactorEntries;
const MIN_SCORE = 0;
const MAX_SCORE = 100;
const HUMAN_MOAT_MEDIUM_THRESHOLD = 40;
const HUMAN_MOAT_STRONG_THRESHOLD = 70;
exports.AI_REPLACEMENT_PRESSURE_QUADRANT_THRESHOLD = 25;
exports.HUMAN_MOAT_QUADRANT_THRESHOLD = 55;
exports.HUMAN_MOAT_FACTOR_DEFINITIONS = [
    {
...
function getOccupationSignalText(occupation) {
    return [
...
function getProvisionalMajorGroupProfile(occupation) {
...
function applySignalRules(base, text, rules) {
    rules.forEach((rule) => {
...
function applyTaskShapeAdjustments(base, occupation) {
...
function getQuadrantKey(pressure, humanMoatScore) {
    if (pressure <= exports.AI_REPLACEMENT_PRESSURE_QUADRANT_THRESHOLD && humanMoatScore >= exports.HUMAN_MOAT_QUADRANT_THRESHOLD)
        return "human_moat_zone";
    if (pressure > exports.AI_REPLACEMENT_PRESSURE_QUADRANT_THRESHOLD && humanMoatScore >= exports.HUMAN_MOAT_QUADRANT_THRESHOLD)
        return "tug_zone";
    if (pressure <= exports.AI_REPLACEMENT_PRESSURE_QUADRANT_THRESHOLD && humanMoatScore < exports.HUMAN_MOAT_QUADRANT_THRESHOLD)
        return "automation_zone";
    return "danger_zone";
}
function localized(language, value) {
    return language === "en" ? value.en : value.zh;
}
function normalizeAirsScore(value) {
...
function calculateHumanMoatScore(factors) {
    const weightedScore = exports.HUMAN_MOAT_FACTOR_DEFINITIONS.reduce((sum, definition) => sum + clampScore(factors[definition.key]) * definition.weight, 0);
    return roundScore(weightedScore);
}
function getHumanMoatFactors(occupation) {
    /**
...
function getHumanMoatScore(occupation) {
    return calculateHumanMoatScore(getHumanMoatFactors(occupation));
}
function getHumanMoatLevel(score) {
    if (score < HUMAN_MOAT_MEDIUM_THRESHOLD)
        return "weak";
    if (score < HUMAN_MOAT_STRONG_THRESHOLD)
        return "medium";
    return "strong";
}
function getHumanMoatLabel(score, language = "zh") {
...
function getAiReplacementPressure(occupation) {
    return roundScore(100 - normalizeAirsScore(occupation.airs));
}
function getOccupationQuadrant(occupation, language = "zh") {
...
function getQuadrantMetaByKey(key, language = "zh") {
    return {
        key,
        ...localized(language, QUADRANT_CONTENT[key])
    };
}
function getOccupationUniverseMetrics(occupation, language = "zh") {
...
function getHumanMoatFactorEntries(occupation, language = "zh") {
    const factors = getHumanMoatFactors(occupation);
    return exports.HUMAN_MOAT_FACTOR_DEFINITIONS.map((definition) => ({
        key: definition.key,
